                    ...
                    if is_cart_item_exists:
                         ...
                         for pr in product_variation:
                              if pr in ex_var_list:
                                   ...
                                   item_id = id[index]
                                   item = Cartitem.objects.get(product = pr, id = item_id )
                                   item.quantity +=1
                                   item.user = user
                                   item.save()
                              
                              else:
                                   cart_item = Cartitem.object.filter(cart=cart)
...
def register(request):
     ...
     if request.method=="POST":
          ...
          if form.is_valid():
               ...
               user = Accounts.objects.create_user(first_name=first_name, last_name=last_name, email=email, username=username, password=password,phone_number = phone_number)
               ...
               return redirect(f'/accounts/otp/{user.id}')
